...
const getTranslationsFileSchema = (filename: string) =>
  createRequiredObjSchema(filename, getTranslationKeysSchema(), z.optional(z.string()));

const getTranslationFilename = (lang: string) => `translations.${lang}.json`;

export type TranslationsFileType = z.infer<ReturnType<typeof getTranslationsFileSchema>>;
...
export const initializeTranslationsForLocale = (lang: LanguageCode): TranslationsFileType => {
  const fullPath = resolve(TRANSLATIONS_DIR, getTranslationFilename(lang));
  if (!existsSync(fullPath)) {
    throw new Error(`the translations file does not exists for "${lang}" locale`, {
      cause: {
  ...
  const translations = getTranslationsFileSchema(fullPath).parse(JSON.parse(content));
  return translations;
};
...
export const createTranslationsFileForLocale = (lang: string): void => {
  ...
  const oldContent = existsSync(fullPath) ? readFileSync(fullPath, { encoding: "utf-8" }) : "{}";
  const data: Record<string, string> = JSON.parse(oldContent);

  const translationKeys = Object.values(TranslationKeys).sort((a, b) => a.localeCompare(b));
  const content = translationKeys.reduce<Partial<Record<TranslationKey, "">>>((acc, key) => {
    return {
      ...acc,
      [key]: data[key] ?? "",
    };
